...
// Hash to string (HEX)
string hashToString(const Hash& hash) {
    stringstream ss;
    for(int i = 0; i < SHA1HASHLENGTHUINTS; i++) {
        ss << hex << setw(8) << setfill('0') << hash.c[i];
    }
    return ss.str();
}

// String to Hash
Hash stringToHash(const string& str) {
    Hash hash;
    for(int i = 0; i < SHA1HASHLENGTHUINTS; i++) {
        string byteStr = str.substr(i * 8, 8);
        hash.c[i] = stoul(byteStr, nullptr, 16);
    }
    return hash;
}